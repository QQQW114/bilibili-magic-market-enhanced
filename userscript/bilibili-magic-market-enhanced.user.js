// ==UserScript==
// @name         Bilibili 魔力赏市集搜索增强
// @namespace    local.bilibili.magic-market-enhanced
// @version      0.4.0
// @description  为 B 站魔力赏/市集页面添加连续慢速扫描、结果临时缓存和当前页筛选。
// @match        https://mall.bilibili.com/neul-next/index.html?page=magic-market_index*
// @match        https://mall.bilibili.com/neul-next/index.html?page=magic-market_search-result*
// @run-at       document-idle
// @grant        none
// ==/UserScript==

(function injectBmmSearchEnhancerStyle() {
  const style = document.createElement('style');
  style.textContent = `#bmm-enhancer,
#bmm-enhancer * {
...
#bmm-enhancer .bmm-card,
#bmm-enhancer .bmm-results,
#bmm-enhancer .bmm-toast {
  pointer-events: auto;
}

#bmm-enhancer .bmm-card {
  overflow: hidden;
  border: 1px solid rgba(120, 109, 246, 0.24);
  border-radius: 16px;
  background: rgba(255, 255, 255, 0.96);
  box-shadow: 0 12px 30px rgba(20, 22, 40, 0.18);
  backdrop-filter: blur(14px);
}

#bmm-enhancer .bmm-head,
#bmm-enhancer .bmm-results-head {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 10px;
  padding: 10px 12px;
  background: linear-gradient(135deg, #786df6, #ff7eb3);
  color: #fff;
}

#bmm-enhancer .bmm-title,
#bmm-enhancer .bmm-results-title {
  font-size: 15px;
  font-weight: 700;
  line-height: 1.2;
}

#bmm-enhancer .bmm-subtitle,
#bmm-enhancer .bmm-results-subtitle {
  margin-top: 2px;
  font-size: 11px;
  opacity: 0.9;
}

#bmm-enhancer .bmm-icon-btn {
  flex: 0 0 auto;
  width: 28px;
  height: 28px;
  border: 0;
...
#bmm-enhancer .bmm-body {
  padding: 10px 12px 12px;
}

#bmm-enhancer.bmm-collapsed .bmm-body {
  display: none;
}

#bmm-enhancer .bmm-input-wrap {
  display: flex;
  align-items: center;
  gap: 6px;
  min-height: 38px;
...
#bmm-enhancer .bmm-search-icon {
  color: #786df6;
  font-size: 18px;
  font-weight: 700;
}

#bmm-enhancer .bmm-input {
  min-width: 0;
  flex: 1;
  height: 34px;
  border: 0;
...
#bmm-enhancer .bmm-btn,
#bmm-enhancer .bmm-load-more {
  min-height: 34px;
...
#bmm-enhancer .bmm-primary {
  border-color: #786df6;
  background: #786df6;
  color: #fff;
}

#bmm-enhancer.bmm-loading .bmm-primary {
  opacity: 0.78;
  cursor: wait;
}

#bmm-enhancer .bmm-stats {
  margin-top: 8px;
  color: #61666d;
  font-size: 12px;
}

#bmm-enhancer .bmm-hint {
  margin-top: 4px;
  color: #9499a0;
  font-size: 11px;
  line-height: 1.4;
...
#bmm-enhancer .bmm-results {
  ...
  overflow: hidden;
  border: 1px solid rgba(120, 109, 246, 0.18);
  border-radius: 18px;
  background: #f5f5f7;
  box-shadow: 0 18px 48px rgba(20, 22, 40, 0.26);
}

#bmm-enhancer .bmm-results-body {
  max-height: calc(100vh - 160px - env(safe-area-inset-top, 0px));
  overflow: auto;
  padding: 10px;
}

#bmm-enhancer .bmm-message {
  padding: 18px 12px;
  border-radius: 12px;
  background: #fff;
  color: #61666d;
...
#bmm-enhancer .bmm-message.bmm-error {
  color: #d33;
}

#bmm-enhancer .bmm-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 8px;
}

#bmm-enhancer .bmm-result-card {
  overflow: hidden;
  border-radius: 12px;
  background: #fff;
...
#bmm-enhancer .bmm-result-card:hover {
  transform: translateY(-1px);
}

#bmm-enhancer .bmm-thumb {
  display: flex;
  align-items: center;
...
#bmm-enhancer .bmm-thumb img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

#bmm-enhancer .bmm-no-img {
  color: #9499a0;
  font-size: 12px;
}

#bmm-enhancer .bmm-result-name {
  min-height: 36px;
  padding: 8px 8px 0;
  color: #212121;
  font-size: 13px;
...
#bmm-enhancer .bmm-result-seller {
  padding: 5px 8px 9px;
  color: #61666d;
  font-size: 12px;
  white-space: nowrap;
...
#bmm-enhancer .bmm-hidden {
  display: none !important;
}

#bmm-enhancer .bmm-actions-continuous {
  grid-template-columns: 2fr 1fr;
}

#bmm-enhancer .bmm-danger {
  border-color: #ff6b6b;
  color: #ff4d4f;
}

#bmm-enhancer .bmm-btn:disabled,
#bmm-enhancer .bmm-load-more:disabled {
  opacity: 0.55;
  cursor: not-allowed;
}
`;
